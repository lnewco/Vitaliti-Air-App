...
const AppNavigator = () => {
    ...
    const handleAppStateChange = (nextAppState) => {
      if (nextAppState === 'active') {
        ...
        try {
          ...
          console.log('🔄 Could not check session status');
        }
        ...
        if (isAuthenticated && onboardingState === 'completed') {
          console.log('🔄 Already authenticated and onboarding completed - maintaining current navigation state');
          return;
        }
        ...
        if (!isAuthenticated && onboardingState !== 'completed') {
          console.log('🔄 Not authenticated, checking onboarding status');
          checkOnboardingStatus();
        }
      }
    };
  ...
  const checkOnboardingStatus = async () => {
    try {
      ...
      if (isAuthenticated && onboardingState === 'completed') {
        console.log('🔄 Skipping onboarding check - already authenticated with completed onboarding');
        return;
      }
