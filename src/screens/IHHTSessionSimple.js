...
export default function IHHTSessionSimple() {
  ...
  useEffect(() => {
    ...
    const initSession = async () => {
      if (isInitializing) {
        console.warn('⚠️ Session initialization already in progress, skipping duplicate call');
        return;
      }
      ...
      try {
        ...
        if (isCancelled) {
          console.log('⚠️ Session initialization was cancelled, not starting session');
          return;
        }
        ...
        if (!isCancelled) {
          ...
          if (preSessionData) {
            console.log('📝 Saving pre-session survey data:', preSessionData);
            try {
              ...
              console.log('✅ Pre-session survey saved successfully');
            } catch (error) {
        ...
        console.log('🚨🚨🚨 CODE VERSION 3 - CACHE CLEARED! 🚨🚨🚨');
  ...
  useEffect(() => {
    ...
    console.log('📱 IHHTSession useEffect - isPulseOxConnected:', isPulseOxConnected, 'pulseOximeterData:', pulseOximeterData);
    ...
    if (isPulseOxConnected && pulseOximeterData) {
      ...
      if (pulseOximeterData.spo2 && pulseOximeterData.heartRate && EnhancedSessionManager.isActive) {
        console.log('📱 IHHTSession: Sending reading to EnhancedSessionManager:', {
        ...
        console.log('⚠️ IHHTSession: Skipping data - no active session in manager');
      }
    } else {
            ...
            <TouchableOpacity 
              onPress={() => {
                ...
                if (adaptiveInstruction.type === 'dial_adjustment') {
                  console.log('✅ Dial adjustment confirmed - animation will start when masks are switched');
