...
export default function IHHTActiveSession() {
    ...
    if (isSessionActive && !isPaused) {
      phaseTimerRef.current = setInterval(() => {
        setPhaseTimeRemaining(prev => {
          if (prev <= 1) {
            // Return 0 to trigger phase transition in separate effect
            return 0;
          }
          return prev - 1;
        });
  ...
  useEffect(() => {
    if (phaseTimeRemaining === 0 && isSessionActive && !isPaused) {
      if (currentPhase === 'altitude') {
        // Switch to recovery
  ...
  }, [phaseTimeRemaining, isSessionActive, isPaused, currentPhase, currentCycle, completeSession]);
    ...
    Alert.alert(
      ...
      [
        ...
        { 
          ...
          style: 'default',
          onPress: () => {
            // Set timer to 0 to trigger phase transition
