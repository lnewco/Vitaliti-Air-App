...
const PhoneVerificationScreen = ({ route, navigation }) => {
  ...
  const handleVerifyOTP = async (otpCodeOverride = null) => {
    ...
    console.log('🔍 Verify button pressed - OTP:', otpToUse, 'State:', otpCode);
    
    // Prevent duplicate verification while in progress only
    if (verificationInProgress.current) {
      console.log('⚠️ Verification already in progress');
      return;
    }
    
    // Allow retry of same code after error
    // Only block if we're still processing
    ...
    try {
      // Get the phone number from the correct source
      let formattedPhone;
      if (phoneFromLogin) {
        formattedPhone = phoneFromLogin;
      } else if (phoneNumber) {
      ...
      console.log('📞 Verifying with phone:', formattedPhone, 'and code:', otpToUse);
    ...
    return (
      <>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            // Force read current values from inputs if state is out of sync
            const currentValues = [];
            for (let i = 0; i < 6; i++) {
              const ref = inputRefs.current[i];
              if (ref && ref._lastNativeText) {
                currentValues.push(ref._lastNativeText);
              } else {
                currentValues.push(otpCode[i] || '');
              }
            }
            const manualCode = currentValues.join('');
            console.log('📲 Button tap - Manual code:', manualCode, 'State code:', otpCode.join(''));
            
            if (manualCode.length === 6) {
              // Update state and verify
              setOtpCode(currentValues);
              handleVerifyOTP(manualCode);
            } else {
              handleVerifyOTP();
            }
          }}
