...
import IHHTMetricsTracker from './IHHTMetricsTracker';
...
class EnhancedSessionManager {
  ...
  async startSession(sessionId, protocolConfig = {}) {
    ...
    if (this.sessionStartInProgress) {
      console.warn('⚠️ Session start already in progress, returning existing promise');
    ...
    this.sessionStartPromise = this._startSessionInternal(sessionId, protocolConfig);
    
    try {
      const result = await this.sessionStartPromise;
      return result;
    } finally {
      this.sessionStartInProgress = false;
      this.sessionStartPromise = null;
  ...
  async _startSessionInternal(sessionId, protocolConfig = {}) {
    ...
    try {
      ...
      // Initialize currentSession BEFORE trying to use it
      this.currentSession = {
        id: sessionId,
        supabaseId: null,  // Will be set after Supabase creation
        startTime: new Date().toISOString()
      };
      
      // Initialize metrics tracking
      IHHTMetricsTracker.startSession(sessionId);
      ...
      try {
        ...
        // CRITICAL: Store the Supabase UUID for readings
        if (supabaseSession?.id && this.currentSession) {
          this.currentSession.supabaseId = supabaseSession.id;
          console.log('📝 Stored Supabase ID for readings:', supabaseSession.id);
        }
      } catch (supabaseError) {
      ...
      // Update supabaseId if we got one, otherwise keep sessionId
      if (!this.currentSession.supabaseId) {
        this.currentSession.supabaseId = sessionId;
      }
      this.isActive = true;
      this.isPaused = false;
  ...
  async advancePhase() {
    ...
    // End the current phase in metrics tracker
    IHHTMetricsTracker.endPhase();
    ...
    if (this.currentPhase === 'ALTITUDE') {
      ...
      await this.recordAdaptiveEvent('recovery_complete', {
        ...
        spo2_value: this.currentSpO2,
        recovery_time: Math.floor((Date.now() - this.phaseStartTime) / 1000),
        max_spo2: this.phaseStats?.maxSpO2 || null
      });
  ...
  async stopSession(reason = 'manual') {
    ...
    if (this.currentSession) {
      // Calculate and save IHHT metrics
      IHHTMetricsTracker.endSession();
  ...
  async addReading(reading) {
    if (!this.isActive || !this.currentSession) {
      console.log('⚠️ Cannot add reading - session not active or no current session', {
    ...
    console.log('📊 Adding reading to session:', {
      spo2: reading.spo2,
      heartRate: reading.heartRate,
      sessionId: this.currentSession.supabaseId || this.currentSession.id
    });

    // Determine data source based on runtime environment
    const isExpoGo = Constants.appOwnership === 'expo';
    const dataSource = isExpoGo ? 'mock' : 'bluetooth';
    
    const enhancedReading = {
      ...reading,
      session_id: this.currentSession.supabaseId || this.currentSession.id,  // Use Supabase UUID
  ...
  async flushReadingBuffer() {
    ...
    if (!this.currentSession) {
      console.warn('⚠️ Cannot flush readings - no active session');
      return;
    }

    console.log(`💾 Flushing ${this.readingBuffer.length} readings to database`);
    
    const readings = [...this.readingBuffer];
    this.readingBuffer = [];

    try {
      // Add session_id to each reading (SupabaseService expects it in the reading object)
      const readingsWithSessionId = readings.map(reading => ({
        ...reading,
        session_id: this.currentSession.supabaseId || this.currentSession.id,
        sessionId: this.currentSession.id // Keep local ID for reference
      }));
      
      // Pass only the readings array (SupabaseService extracts session_id from first reading)
      await SupabaseService.addReadingsBatch(readingsWithSessionId);
      console.log(`✅ Flushed ${readings.length} readings to database for session ${this.currentSession.id}`);
    } catch (error) {
      console.error('❌ Failed to flush readings:', error);
      // Put readings back if failed
  ...
  confirmDialAdjustment(newLevel) {
    ...
    if (this.currentPhase === 'ALTITUDE') {
      ...
      console.log('📌 Stored pending altitude level: ' + newLevel + ' (will apply at next altitude phase)');
    }
  ...
  async processAdaptiveInstruction(spo2Data) {
    ...
    try {
      ...
      // Track metrics for IHHT adaptation
      IHHTMetricsTracker.processReading(spo2Data.spo2, spo2Data.heartRate || null);
      ...
      if (maskLiftInstruction) {
        ...
        // Track mask lift in metrics
        IHHTMetricsTracker.recordMaskLift();
      }
